...
class CameraSceneManager:
    def __init__(self):
        ...
        self.box_id = p.loadURDF(
            "cube.urdf", [-3, 0, 0.5], physicsClientId=self.client_id
        )
        p.changeVisualShape(
            self.box_id, -1, rgbaColor=[1, 0, 0, 1], physicsClientId=self.client_id
        )
        self.bot_id = p.loadSDF(sdf_file_path, physicsClientId=self.client_id)[0]
        p.resetBasePositionAndOrientation(
            self.bot_id, [0, 0, 0.5], [0, 0, 0, 1], physicsClientId=self.client_id
        )
        p.changeVisualShape(
            self.bot_id, 2, rgbaColor=[0, 0, 0, 0], physicsClientId=self.client_id
        )
    ...
    def render_image(self):
        ...
        vm = p.computeViewMatrix(
            cameraEyePosition=cam_coords,
            cameraTargetPosition=np.multiply(cam_coords, [8, 8, 1]),
            cameraUpVector=[0, 0, 1],
        )
        pm = p.computeProjectionMatrixFOV(90, 1, 0.1, 100)
        _, _, image, *_ = p.getCameraImage(
            width=640,
            height=640,
            viewMatrix=vm,
            projectionMatrix=pm,
